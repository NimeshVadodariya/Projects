...
select salary, concat(first_name," ",last_name) "NAME" from employees
where salary >2000 ;

# 7. Display the names and dates of employees with the column headers "Name" and "Start Date" 
select concat(first_name," ",last_name) "NAME" ,hire_date "START_DATE"
from employees;
...
from employees  join jobs on employees.job_id = jobs.job_id
where job_title in ("sales representative","stock clerk") and
salary not in ("2500","3500","5000");

---------------------------------------------------------------------------------------------------------------------------------------------------------------

# 1) Display the maximum, minimum and average salary and commission earned.
select avg(salary),min(salary),max(salary),avg(commission_pct),min(commission_pct),max(commission_pct)
from employees ;
...
SELECT department_name, COUNT(employees.employee_id) AS count
FROM departments
...
else 'NO COMMISSION' end 'Commission PCT' 
from employees;
...
Select m.manager_id,COUNT(e.employee_id) 'No. of Employees.' ,m.department_id
...
select last_name "Employee Name",department_id,salary,concat(first_name," ",last_name) "Manager Name"
from employees e join employees m
on e.employee_id=m.manager_id
group by m.manager_id,e.last_name ;
...
case when commission_pct is not null then commission_pct*(salary*10/100)
...
select last_name, upper(SUBSTRING(last_name,2,3)) 
from employees;
Select last_name,Concat(UPPER(SUBSTRING(last_name,2,1)),LOWER(SUBSTRING(last_name,3,3))) as'2-5th Last name' from employees ;
...
select concat(first_name," ",last_name) "Name" from empployees
where 'Name' ="palindrome";

# 8. Display First names of all employees with initcaps.
select first_name from employees
where initcap= first_name  ;
...
select d.department_name,e.last_name,e.department_id,l.location_id,l.city
...
where state_province like "atlanta";
...
# 9. Creat	e a query that displays the name,job,department name,salary,grade for all employees. Derive grade based on salary(>=50000=A, >=30000=B,<30000=C) 
...
concat(m.first_name, "", m.last_name) 'Manager name',m.hire_date 'man_hire_date' from employees as e left join employees as m
on e.manager_id = m. employee_id
where e.hire_date > m.hire_date
group by e.manager_id,e.first_name, e.last_name, e.hire_date;
...
join locations l on d.location_id = l.location_id where l.state_province = 'ATLANTA';
